...
# Load model, vectorizer, and data once at app start
...
# Fill missing columns or extract info from keywords where possible
...
with st.form(key="recommend_form"):
    ...
    # Three equal columns for optional filters: experience, location, job type
    ...
    # New row of three equal columns for buttons aligned perfectly below filters
    btn_col1, btn_col2, btn_col3 = st.columns(3)
    with btn_col2:
        # Submit button centered with CSS
...
def generate_pdf(dataframe):
    ...
    Returns a BytesIO stream for Streamlit download.
    """
    pdf = FPDF()
    pdf.add_page()
...
def strip_html_tags(text):
    """
    Remove HTML tags from a string to keep PDF clean.
    """
...
# Process form submission after user clicks submit
if submit:
    ...
    # Check if model and vectorizer loaded correctly
        ...
        if keyword_results.empty:
            ...
            # Center the download button below results with same 3 equal columns layout
            col1, col2, col3 = st.columns(3)
            with col2:
                st.download_button(
